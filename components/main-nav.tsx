...
import { removeGuestCartItemByKey } from "@/lib/guestCart";
...
export function MainNav() {
  ...
  useEffect(() => {
    const onCartUpdated = () => {
      if (isLoggedIn) {
        // المستخدم المسجل: هات السلة من الـ API
        fetchCounts();
        fetchCartItems();
      } else {
        // Guest: هات السلة من localStorage
        setCounts(prev => ({
          ...prev,
          cart: getGuestCartCount(),
        }));
    ...
    if (!isLoggedIn) {
      try {
        if (!guestItem?.productId) {
          console.error("Missing guest cart item:", guestItem);
          toast.error(
            language === "ar"
              ? "تعذر تحديد المنتج"
                        ...
                        <div key={item._id} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate">
                              {item.product?.title || item.title || `Product ${item._id?.substring(0, 6)}`}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              {language === "ar"
