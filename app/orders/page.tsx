                                ...
                                <div key={order._id} className={`overflow-hidden border transition-all duration-300 hover:shadow-lg rounded-2xl ${statusConfig.borderColor} bg-white`}>
                                    ...
                                    <div className="p-4 md:p-6">
                                        ...
                                        {order.paymentStatus === 'pending' && (
                                            <div className="mt-4 pt-4 border-t border-slate-100 flex flex-wrap gap-2 items-center justify-between bg-sky-50/50 p-4 rounded-2xl">
                                                <div className="text-xs text-sky-800 font-medium">
                                                    الطلب بانتظار إتمام عملية الدفع لتأكيده.
                                                </div>
                                                <div className="flex gap-2">
                                                    <Button
                                                        size="sm"
                                                        disabled={processingPayment === order._id}
                                                        onClick={() => handleCompletePayment(order._id, 'card')}
                                                        className="rounded-xl text-xs gap-1.5"
                                                    >
                                                        {processingPayment === order._id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CreditCard className="w-3.5 h-3.5" />}
                                                        دفع بالبطاقة
                                                    </Button>
                                                </div>
                                            </div>
